 ...
 *
 * @author gl56, Based on template by Jim Idle - Temporal Wave LLC - jimi@temporal-wave.com
 * @date 01/01/2026
 */
public abstract class AbstractDecaParser extends Parser {
    ...
    public AbstractProgram parseProgramAndManageErrors(PrintStream err) {
        try {
            ...
            if (msg != null && (msg.contains("est trop grand") || msg.contains("est hors des limites"))) {
                err.println(getDecacCompiler().getSource().getAbsolutePath() + ":" + msg);
                return null;
            }
            throw e;
        } 
